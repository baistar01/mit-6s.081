...
int uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for (i = 0; i < sz; i += PGSIZE)
  {
    ...
    *pte = *pte | PTE_COW;
    ...
    flags = PTE_FLAGS(*pte);
    if (mappages(new, i, PGSIZE, (uint64)pa, flags) != 0) // 直接将父进程物理页映射到子进程中
    {
      goto err;
    }
    incr((void *)pa);
...
int is_cow_fault(pagetable_t pagetable, uint64 va)
{
  ...
  pte_t *pte = walk(pagetable, va, 0); // 获取pte进行合法性检测及识别cow错误
  if (pte == 0)
    return 0;
  if ((*pte & PTE_V) == 0)
    return 0;
...
int cow_alloc(pagetable_t pagetable, uint64 va)
{
  ...
  uvmunmap(pagetable, va, 1, 1);    // 清除原有映射（原有映射权限存在问题）

  flag &= ~(PTE_COW); // 清除COW权限
  flag |= PTE_W;      // 设置写权限
